...
// battles with the forces of evil. Different races will certainly be involved. Each race has a certain worth when
// battling against others. On the side of good we have the following races, with their associated worth:
//     Hobbits: 1
// Men: 2
// Elves: 3
// Dwarves: 3
...
const goodVsEvil = (good, evil) => {
    const worthG = [1,2,3,3,4,10]
    const worthE = [1,2,2,2,3,5,10]
    const gF = good.split(' ').reduce((a, c, idx) => a + +c * worthG[idx], 0)
    const eF =  evil.split(' ').reduce((a, c, idx) => a + +c * worthE[idx], 0)
    if (gF === eF) return 'Battle Result: No victor on this battle field'
    if (gF > eF) return 'Battle Result: Good triumphs over Evil'
    if( gF < eF) return 'Battle Result: Evil eradicates all trace of Good'
};
