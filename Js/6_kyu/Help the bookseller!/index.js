...
const stockList = (listOfArt, listOfCat) => {
    const categorized = Object.entries(
        listOfArt.reduce((a,c) => (t = c.split(' '), (a[t[0][0]] || a[t[0][0]] === 0) && (a[t[0][0]] = a[t[0][0]] + +t[1]), a)
        ,listOfCat.reduce((ac, cu) => (ac[cu] = 0, ac), {})))
    if (categorized.reduce((acc, curr) => acc + curr[1], 0) === 0) return ''
    return categorized.map((item) => `(${item[0]} : ${item[1]})`).join(' - ')
}
